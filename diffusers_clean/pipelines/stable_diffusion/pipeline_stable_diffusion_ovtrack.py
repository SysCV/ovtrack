...
def undo(img_out, t, betas):
    beta = _extract_into_tensor(betas, t, img_out.shape).to(img_out.device)
    img_in_est = torch.sqrt(1 - beta) * img_out + \
        torch.sqrt(beta) * torch.randn_like(img_out)

    return img_in_est

class StableDiffusionRepaintPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        # num_train_timesteps is 1000 by default
        times = torch.tensor(get_schedule_jump(num_inference_steps, n_sample=n_sample, jump_length=jump_length,
                                                jump_n_sample=jump_n_sample)) * (
                self.scheduler.num_train_timesteps // num_inference_steps) + offset
        # timesteps is the start timestep, this is to find the first start point in times series
        # print("((times == timesteps[0].cpu()).nonzero(as_tuple=True)[0])[0]")
        t_start = ((times == timesteps[0].cpu()).nonzero(as_tuple=True)[0])[0]
        # print("t_start = max(num_inference_steps - init_timestep + offset, 0)")
        ...
        for t_last, t_cur in time_pairs:
            ...
            if t_cur < t_last:
                ...
                noise_pred = self.unet(latent_model_input, t_last, encoder_hidden_states=text_embeddings).sample
                ...
                latents = self.scheduler.step(noise_pred, t_last, latents, **extra_step_kwargs).prev_sample

                # masking
                if t_cur > harmonize_steps:
                    # t_cur = torch.tensor(0)
                    if t_cur <= 0:
                        init_latents_proper = init_latents_orig
                    else:
                        init_latents_proper = self.scheduler.add_noise(init_latents_orig, noise, t_cur)
                        # init_latents_proper = init_latents_orig
                    # repaint background
                ...
                t = t_last
                while(t < t_cur):
                    latents = undo(latents, t, betas=self.scheduler.betas)
                    t += 1
